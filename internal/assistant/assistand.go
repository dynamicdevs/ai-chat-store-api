...
func New(db *database.Connection, openia *openia.Openia) *Assistant {
	return &Assistant{
		db,
		openia,
	}
...
func (a *Assistant) HelpWithEveryThing(messages chat.Messages) (chat.Messages, error) {
...
func (a *Assistant) HelpWithEveryThingProduct(messages chat.Messages) (chat.Messages, error) {
	//get embedding of the question
	ctx := context.Background()
	question := messages[len(messages)-1].Content
	messages[len(messages)-1].Content = fmt.Sprintf(`Question:%s`, question)
